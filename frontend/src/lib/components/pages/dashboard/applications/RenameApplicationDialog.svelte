<script lang="ts">
  ...
  import { editImage, type ImageCreateRequest } from '$lib/service/imageService';
  ...
  export const open = (oid: number, initial: Partial<ImageCreateRequest> = {}) => {
    id = oid;
    data = {
      name: '',
      ...initial
...
<DefaultDialog bind:this={dialog}>
  <svelte:fragment slot="title">Rename Application</svelte:fragment>
